...
#include "flowdis/batadv_packet.h"
#include "flowdis/bitops.h"
#include "flowdis/byteorder.h"
#include "flowdis/dsfield.h"
#include "flowdis/ether.h"
#include "flowdis/fc_fcoe.h"
#include "flowdis/flow_dissector.h"
#include "flowdis/gre.h"
#include "flowdis/kernel-compat.h"
#include "flowdis/pptp.h"
#include "flowdis/sctp.h"
#include "flowdis/skbuff.h"
#include "flowdis/vlan.h"

#endif /* !ORIGKERNEL */

struct flow_dissect_internal {
	enum flow_dissect_ret class;
	const char *badmsg;
};

#define FLOW_DISSECT_INTERNAL_OUT_GOOD() (				\
	(struct flow_dissect_internal) {				\
		.class = FLOW_DISSECT_RET_OUT_GOOD			\
	}								\
)

#define FLOW_DISSECT_INTERNAL_OUT_BAD(msg) (				\
	(struct flow_dissect_internal) {				\
		.class = FLOW_DISSECT_RET_OUT_BAD,			\
		.badmsg = (msg)						\
	}								\
)

#define FLOW_DISSECT_INTERNAL_PROTO_AGAIN() (				\
	(struct flow_dissect_internal) {				\
		.class = FLOW_DISSECT_RET_PROTO_AGAIN			\
	}								\
)

#define FLOW_DISSECT_INTERNAL_IPPROTO_AGAIN() (				\
	(struct flow_dissect_internal) {				\
		.class = FLOW_DISSECT_RET_IPPROTO_AGAIN			\
	}								\
)

#define FLOW_DISSECT_INTERNAL_CONTINUE() (				\
	(struct flow_dissect_internal) {				\
		.class = FLOW_DISSECT_RET_CONTINUE			\
	}								\
)
...
#else
static void skb_flow_get_icmp_tci(const struct sk_buff *skb,
				  struct flow_dissector_key_icmp *key_icmp,
				  void *data, int thoff, int hlen)
#endif
{
...
static struct flow_dissect_internal
			...
			void *target_container, void *data, int nhoff, int hlen)
{
	...
	if (!dissector_uses_key(flow_dissector,
		...
		return FLOW_DISSECT_INTERNAL_OUT_GOOD();

	hdr = __skb_header_pointer(skb, nhoff, sizeof(_hdr), data,
				   hlen, &_hdr);
	if (!hdr)
		return FLOW_DISSECT_INTERNAL_OUT_BAD(
					"__skb_header_pointer failed (MPLS)");
		       ...
		       void *target_container, void *data, int nhoff, int hlen)
{
	...
	if (!arp)
		return FLOW_DISSECT_INTERNAL_OUT_BAD(
					"__skb_header_pointer failed (ARP 1)");

	if (arp->ar_hrd != htons(ARPHRD_ETHER))
		return(FLOW_DISSECT_INTERNAL_OUT_BAD("non-Ethernet ARP"));
	if (arp->ar_pro != htons(ETH_P_IP))
		return(FLOW_DISSECT_INTERNAL_OUT_BAD("non-IP ARP"));
	if (arp->ar_hln != ETH_ALEN)
		return(FLOW_DISSECT_INTERNAL_OUT_BAD("ARP hln wrong"));
	if (arp->ar_pln != 4)
		return(FLOW_DISSECT_INTERNAL_OUT_BAD("ARP pln wrong"));
	if ((arp->ar_op != htons(ARPOP_REPLY)) &&
	    (arp->ar_op != htons(ARPOP_REQUEST)))
		return(FLOW_DISSECT_INTERNAL_OUT_BAD(
					"ARP neither reply nor request"));
	...
	if (!arp_eth)
		return FLOW_DISSECT_INTERNAL_OUT_BAD(
				"__skb_header_pointer failed (ARP 2)");
		       ...
		       unsigned int flags)
{
	...
	if (!hdr)
		return FLOW_DISSECT_INTERNAL_OUT_BAD(
					"__skb_header_pointer failed (GRE)");
	...
	*p_proto = hdr->protocol;
	...
	if (hdr->flags & GRE_KEY) {
		...
		if (!keyid)
			return FLOW_DISSECT_INTERNAL_OUT_BAD(
				"__skb_header_pointer failed (GRE key)");
	...
	if (gre_ver == 0) {
		if (*p_proto == htons(ETH_P_TEB)) {
			...
			if (!eth)
				return FLOW_DISSECT_INTERNAL_OUT_BAD(
				   "__skb_header_pointer failed (GRE v0 TEB)");
			*p_proto = eth->h_proto;
			offset += sizeof(*eth);
		...
		if (!ppp_hdr)
			return FLOW_DISSECT_INTERNAL_OUT_BAD(
					"__skb_header_pointer failed (GRE v1)");
	...
	*p_nhoff += offset;
	...
	return FLOW_DISSECT_INTERNAL_PROTO_AGAIN();
}
			  ...
			  unsigned int flags)
{
	...
	if (!hdr)
		return FLOW_DISSECT_INTERNAL_OUT_BAD(
				"__skb_header_pointer failed (batman)");

	if (hdr->batadv_unicast.version != BATADV_COMPAT_VERSION)
		return FLOW_DISSECT_INTERNAL_OUT_BAD("batman version wrong");

	if (hdr->batadv_unicast.packet_type != BATADV_UNICAST)
		return FLOW_DISSECT_INTERNAL_OUT_BAD("batman not unicast");
		       ...
		       void *target_container, void *data, int thoff, int hlen)
{
	...
	key_tcp = skb_flow_dissector_target(flow_dissector,
					    ...
					    target_container);
	key_tcp->flags = tcp_flag_word(th) & htons(0x0FFF);
}
...
#ifndef ORIGKERNEL
		if (nhoff < sizeof(_eth)) {
			fdret = FLOW_DISSECT_INTERNAL_OUT_BAD("hdr offset too "
							      "small");
			goto out_bad;
		}

		eth = __skb_header_pointer(skb, 0, sizeof(_eth), data, hlen,
					   &_eth);
		if (!eth) {
			fdret = FLOW_DISSECT_INTERNAL_OUT_BAD(
				"__skb_header_pointer failed (Ethernet)");
			goto out_bad;
		}
#endif
	...
	case htons(ETH_P_IP): {
		...
		if (ip_is_fragment(iph)) {
			...
			if (iph->frag_off & htons(IP_OFFSET)) {
				fdret = FLOW_DISSECT_INTERNAL_OUT_GOOD();
				break;
			} else {
				key_control->flags |= FLOW_DIS_FIRST_FRAG;
				if (!(flags &
					...
					fdret =
					    FLOW_DISSECT_INTERNAL_OUT_GOOD();
					break;
				}
			}
		}
...
#endif
		/* struct pppoe_hdr is a variable length structure, that is
		 * the last field is array with zero elements (tag[0]).
		 * Use diagnostic ignored pragma to disable warning that
		 * fields follow a variable length structure. Note that this
		 * code came fomr Linux kernel so we didn't introduce a new
		 * problem
		 */
		struct {
...
#pragma GCC diagnostic pop
	...
	case htons(ETH_P_TIPC): {
		...
		if (!hdr) {
			fdret = FLOW_DISSECT_INTERNAL_OUT_BAD(
					"__skb_header_pointer failed (TIPC)");
			break;
		}
		...
		if (dissector_uses_key(flow_dissector,
			...
			key_addrs = skb_flow_dissector_target(flow_dissector,
			      FLOW_DISSECTOR_KEY_TIPC, target_container);
	...
	case htons(ETH_P_FCOE):
		if ((hlen - nhoff) < FCOE_HEADER_LEN) {
			fdret = FLOW_DISSECT_INTERNAL_OUT_BAD("FCOE header "
							      "len");
			break;
		}
	...
	default:
		fdret = FLOW_DISSECT_INTERNAL_OUT_BAD("unrecognized Ethernet "
						      "protocol");
		break;
	}

	/* Process result of proto processing */
	switch (FLOW_DISSECT_INTERNAL__CLASS(fdret)) {
...
u32 __skb_get_hash_symmetric(const struct sk_buff *skb)
{
	...
	__skb_flow_dissect(NULL, skb, &flow_keys_dissector_symmetric,
			   ...
			   FLOW_DISSECTOR_F_STOP_AT_FLOW_LABEL);
#else
	__skb_flow_dissect(skb, &flow_keys_dissector_symmetric,
			   &keys, NULL, 0, 0, 0,
			   FLOW_DISSECTOR_F_STOP_AT_FLOW_LABEL);
#endif
...
u32 skb_get_poff(const struct sk_buff *skb)
{
	...
	if (!skb_flow_dissect_flow_keys_basic(skb, &keys, NULL,
					      0, 0, 0, 0))
		return 0;
...
#else
int __init init_default_flow_dissectors(void)
#endif
{
...
void flowdis_print_metadata(const struct flow_keys *flow)
{
	...
	case FLOW_DISSECTOR_KEY_IPV4_ADDRS:
		printf("IPv4 source address: %s\n",
		       inet_ntop(AF_INET, &flow->addrs.v4addrs.src,
				 a4buf, sizeof(a4buf)));
		printf("IPv4 destination address: %s\n",
		       inet_ntop(AF_INET, &flow->addrs.v4addrs.dst,
				 a4buf, sizeof(a4buf)));
		break;
	case FLOW_DISSECTOR_KEY_IPV6_ADDRS:
		printf("IPv6 source address: %s\n",
		       inet_ntop(AF_INET6, &flow->addrs.v6addrs.src,
				 abuf, sizeof(abuf)));
		printf("IPv6 destination address: %s\n",
		       inet_ntop(AF_INET6, &flow->addrs.v6addrs.dst,
				 abuf, sizeof(abuf)));
			break;
		default:
			break;
