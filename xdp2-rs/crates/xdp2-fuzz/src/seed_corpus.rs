...
pub fn ipv4_header(protocol: u8, payload: &[u8]) -> Vec<u8> {
    ...
    let mut hdr = vec![
        0x45,
        0x00, // ver=4, ihl=5, tos=0
        (total_len >> 8) as u8,
        total_len as u8, // total length
        0x00,
        0x01,
        0x00,
        0x00, // id, flags, frag_off
        0x40,
        protocol,
        0x00,
        0x00, // ttl=64, proto, checksum
        0x0A,
        0x00,
        0x00,
        0x01, // src: 10.0.0.1
...
pub fn generate_seeds() -> Vec<(&'static str, Vec<u8>)> {
    ...
    vec![
        ...
        (
            "eth_ipv4_icmp",
            eth_frame(0x0800, &ipv4_header(1, &[8, 0, 0, 0, 0, 1, 0, 1])),
        ),
        (
            "eth_ipv4_sctp",
            eth_frame(0x0800, &ipv4_header(132, &[0; 12])),
        ),
        ...
        (
            ...
            eth_frame(
                0x0806,
                &[
                    0, 1, 0x08, 0x00, 6, 4, 0, 1, 0, 0, 0, 0, 0, 0, 10, 0, 0, 1, 0, 0, 0, 0, 0, 0,
                    10, 0, 0, 2,
                ],
            ),
        ),
        (
            ...
            eth_frame(
                0x8035,
                &[
                    0, 1, 0x08, 0x00, 6, 4, 0, 3, 0, 0, 0, 0, 0, 0, 10, 0, 0, 1, 0, 0, 0, 0, 0, 0,
                    10, 0, 0, 2,
                ],
            ),
        ),
        ...
        (
            "eth_ipv4_esp",
            eth_frame(0x0800, &ipv4_header(50, &[0; 16])),
        ),
        (
            "eth_ipv4_ah",
            eth_frame(
...
mod tests {
    ...
    fn seed_corpus_generates_all_seeds() {
        ...
        assert!(
            seeds.len() >= 20,
            "Expected at least 20 seeds, got {}",
            seeds.len()
        );
