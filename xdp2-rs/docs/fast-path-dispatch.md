...
Reproducibility: all three steps get Nix targets alongside
`perf-graph-enum-compare` so a dedicated-machine run can replay the whole
sequence and produce `perf-results/fast-path/` with before/after numbers.
