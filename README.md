...
```
# sudo apt-get install -y build-essential gcc-multilib pkg-config bison flex libboost-all-dev libpcap-dev
```
...
```
# sudo apt-get install -y libelf-dev clang llvm libbpf-dev linux-tools-$(uname -r)
```
