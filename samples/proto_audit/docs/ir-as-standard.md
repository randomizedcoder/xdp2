...
requirement of reconciling six independent implementations of the same RFCs.
...
- **Cross-source comparison** of 38 protocols demonstrates that the IR
  can represent all six sources' definitions precisely enough to detect
  real disagreements (not just formatting differences).

- **TOML-driven extensibility**: 7 mapping files with 200+ entries were
  sufficient to map all six sources without modifying the IR schema itself.
