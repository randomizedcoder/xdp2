//! Protocol name mapping across all four sources.
//!
//! Each source uses different names for the same protocol. This module
//! provides a canonical mapping table so extractors can normalize protocol
//! names to a single canonical form.

use std::collections::HashMap;

/// A protocol's names across all sources.
#[derive(Debug, Clone)]
pub struct ProtocolNames {
    /// Canonical display name (e.g., "IPv4")
    pub canonical: &'static str,
    /// XDP2 proto_def variable name (e.g., "xdp2_parse_ipv4")
    pub xdp2: Option<&'static str>,
    /// Linux kernel UAPI struct name (e.g., "iphdr")
    pub kernel_struct: Option<&'static str>,
    /// Linux kernel header include (e.g., "linux/ip.h")
    pub kernel_header: Option<&'static str>,
    /// Scapy class name (e.g., "IP")
    pub scapy: Option<&'static str>,
    /// tshark dissector filter name (e.g., "ip")
    pub tshark: Option<&'static str>,
    /// etherparse Rust struct name (e.g., "Ethernet2Header")
    pub etherparse_struct: Option<&'static str>,
    /// etherparse source file (relative to crate root, e.g., "etherparse/src/link/ethernet2_header.rs")
    pub etherparse_file: Option<&'static str>,
    /// libpcap name for gencode/struct lookup (e.g., "IPv4", "vlan_tag")
    pub libpcap_name: Option<&'static str>,
    /// libpcap source file (e.g., "gencode.c", "pcap/vlan.h")
    pub libpcap_file: Option<&'static str>,
    /// Minimum header size in bytes
    pub min_header_bytes: u32,
    /// Whether header length is variable
    pub variable_length: bool,
}

/// Build the complete protocol name mapping table.
pub fn protocol_table() -> Vec<ProtocolNames> {
    vec![
        // ── Layer 2 ──
        ProtocolNames {
            canonical: "Ethernet",
            xdp2: Some("xdp2_parse_ether"),
            kernel_struct: Some("ethhdr"),
            kernel_header: Some("linux/if_ether.h"),
            scapy: Some("Ether"),
            tshark: Some("eth"),
            etherparse_struct: Some("Ethernet2Header"),
            etherparse_file: Some("etherparse/src/link/ethernet2_header.rs"),
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 14,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "VLAN",
            xdp2: Some("xdp2_parse_vlan"),
            kernel_struct: Some("vlan_hdr"),
            kernel_header: Some("linux/if_vlan.h"),
            scapy: Some("Dot1Q"),
            tshark: Some("vlan"),
            etherparse_struct: Some("SingleVlanHeader"),
            etherparse_file: Some("etherparse/src/link/single_vlan_header.rs"),
            libpcap_name: Some("vlan_tag"),
            libpcap_file: Some("pcap/vlan.h"),
            min_header_bytes: 4,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "PBB",
            xdp2: Some("xdp2_parse_pbb"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("PBB"),
            tshark: Some("ieee8021ah"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 18,
            variable_length: false,
        },
        // ── Layer 3 ──
        ProtocolNames {
            canonical: "IPv4",
            xdp2: Some("xdp2_parse_ipv4"),
            kernel_struct: Some("iphdr"),
            kernel_header: Some("linux/ip.h"),
            scapy: Some("IP"),
            tshark: Some("ip"),
            etherparse_struct: Some("Ipv4Header"),
            etherparse_file: Some("etherparse/src/net/ipv4_header.rs"),
            libpcap_name: Some("IPv4"),
            libpcap_file: Some("gencode.c"),
            min_header_bytes: 20,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "IPv6",
            xdp2: Some("xdp2_parse_ipv6"),
            kernel_struct: Some("ipv6hdr"),
            kernel_header: Some("linux/ipv6.h"),
            scapy: Some("IPv6"),
            tshark: Some("ipv6"),
            etherparse_struct: Some("Ipv6Header"),
            etherparse_file: Some("etherparse/src/net/ipv6_header.rs"),
            libpcap_name: Some("IPv6"),
            libpcap_file: Some("gencode.c"),
            min_header_bytes: 40,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "ARP",
            xdp2: Some("xdp2_parse_arp"),
            kernel_struct: Some("arphdr"),
            kernel_header: Some("linux/if_arp.h"),
            scapy: Some("ARP"),
            tshark: Some("arp"),
            etherparse_struct: Some("ArpPacket"),
            etherparse_file: Some("etherparse/src/net/arp_packet.rs"),
            libpcap_name: Some("ARP"),
            libpcap_file: Some("gencode.c"),
            min_header_bytes: 8,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "RARP",
            xdp2: Some("xdp2_parse_rarp"),
            kernel_struct: Some("arphdr"),
            kernel_header: Some("linux/if_arp.h"),
            scapy: Some("ARP"),
            tshark: Some("rarp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: Some("RARP"),
            libpcap_file: Some("gencode.c"),
            min_header_bytes: 8,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "ICMPv4",
            xdp2: Some("xdp2_parse_icmpv4"),
            kernel_struct: Some("icmphdr"),
            kernel_header: Some("linux/icmp.h"),
            scapy: Some("ICMP"),
            tshark: Some("icmp"),
            etherparse_struct: Some("Icmpv4Header"),
            etherparse_file: Some("etherparse/src/transport/icmpv4_header.rs"),
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "ICMPv6",
            xdp2: Some("xdp2_parse_icmpv6"),
            kernel_struct: Some("icmp6hdr"),
            kernel_header: Some("linux/icmpv6.h"),
            scapy: Some("ICMPv6Unknown"),
            tshark: Some("icmpv6"),
            etherparse_struct: Some("Icmpv6Header"),
            etherparse_file: Some("etherparse/src/transport/icmpv6_header.rs"),
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "IGMP",
            xdp2: Some("xdp2_parse_igmp"),
            kernel_struct: Some("igmphdr"),
            kernel_header: Some("linux/igmp.h"),
            scapy: Some("IGMP"),
            tshark: Some("igmp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: false,
        },
        // ── Layer 4 ──
        ProtocolNames {
            canonical: "TCP",
            xdp2: Some("xdp2_parse_tcp_notlvs"),
            kernel_struct: Some("tcphdr"),
            kernel_header: Some("linux/tcp.h"),
            scapy: Some("TCP"),
            tshark: Some("tcp"),
            etherparse_struct: Some("TcpHeader"),
            etherparse_file: Some("etherparse/src/transport/tcp_header.rs"),
            libpcap_name: Some("TCP"),
            libpcap_file: Some("gencode.c"),
            min_header_bytes: 20,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "UDP",
            xdp2: Some("xdp2_parse_udp"),
            kernel_struct: Some("udphdr"),
            kernel_header: Some("linux/udp.h"),
            scapy: Some("UDP"),
            tshark: Some("udp"),
            etherparse_struct: Some("UdpHeader"),
            etherparse_file: Some("etherparse/src/transport/udp_header.rs"),
            libpcap_name: Some("UDP"),
            libpcap_file: Some("gencode.c"),
            min_header_bytes: 8,
            variable_length: false,
        },
        // ── Tunneling ──
        ProtocolNames {
            canonical: "GRE",
            xdp2: Some("xdp2_parse_gre"),
            kernel_struct: Some("gre_base_hdr"),
            kernel_header: Some("linux/gre.h"),
            scapy: Some("GRE"),
            tshark: Some("gre"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "VXLAN",
            xdp2: Some("xdp2_parse_vxlan"),
            kernel_struct: Some("vxlanhdr"),
            kernel_header: Some("linux/vxlan.h"),
            scapy: Some("VXLAN"),
            tshark: Some("vxlan"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "Geneve",
            xdp2: Some("xdp2_parse_geneve"),
            kernel_struct: Some("genevehdr"),
            kernel_header: Some("linux/geneve.h"),
            scapy: Some("GENEVE"),
            tshark: Some("geneve"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "MPLS",
            xdp2: Some("xdp2_parse_mpls"),
            kernel_struct: Some("mpls_label"),
            kernel_header: Some("linux/mpls.h"),
            scapy: Some("MPLS"),
            tshark: Some("mpls"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "PPP",
            xdp2: Some("xdp2_parse_ppp"),
            kernel_struct: None,
            kernel_header: Some("linux/ppp_defs.h"),
            scapy: Some("PPP"),
            tshark: Some("ppp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 2,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "PPPoE",
            xdp2: Some("xdp2_parse_pppoe"),
            kernel_struct: Some("pppoe_hdr"),
            kernel_header: Some("linux/ppp_defs.h"),
            scapy: Some("PPPoE"),
            tshark: Some("pppoes"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 6,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "L2TP",
            xdp2: Some("xdp2_parse_l2tp"),
            kernel_struct: Some("l2tp_control_hdr"),
            kernel_header: Some("linux/l2tp.h"),
            scapy: Some("L2TP"),
            tshark: Some("l2tp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 6,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "ERSPAN",
            xdp2: Some("xdp2_parse_erspan"),
            kernel_struct: Some("erspan_base_hdr"),
            kernel_header: Some("linux/erspan.h"),
            scapy: Some("ERSPAN_II"),
            tshark: Some("erspan"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "NSH",
            xdp2: Some("xdp2_parse_nsh"),
            kernel_struct: Some("nshhdr"),
            kernel_header: Some("linux/nsh.h"),
            scapy: Some("NSH"),
            tshark: Some("nsh"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "HSR",
            xdp2: Some("xdp2_parse_hsr"),
            kernel_struct: Some("hsr_tag"),
            kernel_header: Some("linux/hsr_tag.h"),
            scapy: Some("HSRTag"),
            tshark: Some("hsr"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 6,
            variable_length: false,
        },
        // ── Security ──
        ProtocolNames {
            canonical: "ESP",
            xdp2: Some("xdp2_parse_esp"),
            kernel_struct: Some("ip_esp_hdr"),
            kernel_header: Some("linux/ip.h"),
            scapy: Some("ESP"),
            tshark: Some("esp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "AH",
            xdp2: Some("xdp2_parse_ah"),
            kernel_struct: Some("ip_auth_hdr"),
            kernel_header: Some("linux/ip.h"),
            scapy: Some("AH"),
            tshark: Some("ah"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 12,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "MACsec",
            xdp2: Some("xdp2_parse_macsec"),
            kernel_struct: Some("macsec_sci"),
            kernel_header: Some("linux/if_macsec.h"),
            scapy: Some("MACsecSCI"),
            tshark: Some("macsec"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: false,
        },
        // ── Management ──
        ProtocolNames {
            canonical: "LLDP",
            xdp2: Some("xdp2_parse_lldp"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("LLDPDU"),
            tshark: Some("lldp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 2,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "PTP",
            xdp2: Some("xdp2_parse_ptp"),
            kernel_struct: Some("ptp_header"),
            kernel_header: Some("linux/ptp_classify.h"),
            scapy: Some("PTP"),
            tshark: Some("ptp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 34,
            variable_length: false,
        },
        // ── SRv6 ──
        ProtocolNames {
            canonical: "SRv6",
            xdp2: Some("xdp2_parse_srv6"),
            kernel_struct: Some("ipv6_sr_hdr"),
            kernel_header: Some("linux/seg6.h"),
            scapy: Some("IPv6ExtHdrSegmentRouting"),
            tshark: Some("ipv6.routing.srh"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: true,
        },
        // ── Storage ──
        ProtocolNames {
            canonical: "AoE",
            xdp2: Some("xdp2_parse_aoe"),
            kernel_struct: Some("aoe_hdr"),
            kernel_header: Some("linux/aoe.h"),
            scapy: Some("AOE"),
            tshark: Some("aoe"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 10,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "FCoE",
            xdp2: Some("xdp2_parse_fcoe"),
            kernel_struct: Some("fcoe_hdr"),
            kernel_header: Some("linux/fcoe.h"),
            scapy: Some("FCoE"),
            tshark: Some("fcoe"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 14,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "EtherCAT",
            xdp2: Some("xdp2_parse_ethercat"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("EtherCat"),
            tshark: Some("ecat"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 2,
            variable_length: true,
        },
        // ── Wireless ──
        ProtocolNames {
            canonical: "IEEE802.11",
            xdp2: Some("xdp2_parse_ieee80211"),
            kernel_struct: Some("ieee80211_hdr"),
            kernel_header: Some("linux/ieee80211.h"),
            scapy: Some("Dot11"),
            tshark: Some("wlan"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 24,
            variable_length: true,
        },
        // ── CAN bus ──
        ProtocolNames {
            canonical: "CAN",
            xdp2: Some("xdp2_parse_can"),
            kernel_struct: Some("can_frame"),
            kernel_header: Some("linux/can.h"),
            scapy: Some("CAN"),
            tshark: Some("can"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 16,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "CAN_FD",
            xdp2: Some("xdp2_parse_canfd"),
            kernel_struct: Some("canfd_frame"),
            kernel_header: Some("linux/can.h"),
            scapy: Some("CANFD"),
            tshark: Some("can"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 72,
            variable_length: false,
        },
        // ── Bluetooth ──
        ProtocolNames {
            canonical: "HCI",
            xdp2: Some("xdp2_parse_hci"),
            kernel_struct: Some("hci_command_hdr"),
            kernel_header: Some("net/bluetooth/hci.h"),
            scapy: Some("HCI_Hdr"),
            tshark: Some("bthci_cmd"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 1,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "L2CAP",
            xdp2: Some("xdp2_parse_l2cap"),
            kernel_struct: Some("l2cap_hdr"),
            kernel_header: Some("net/bluetooth/l2cap.h"),
            scapy: Some("L2CAP_Hdr"),
            tshark: Some("btl2cap"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: true,
        },
        // ── InfiniBand ──
        ProtocolNames {
            canonical: "IB_LRH",
            xdp2: Some("xdp2_parse_ib_lrh"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("IB_LRH"),
            tshark: Some("infiniband.lrh"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "IB_GRH",
            xdp2: Some("xdp2_parse_ib_grh"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("IB_GRH"),
            tshark: Some("infiniband.grh"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 40,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "IB_BTH",
            xdp2: Some("xdp2_parse_ib_bth"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("IB_BTH"),
            tshark: Some("infiniband.bth"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 12,
            variable_length: false,
        },
        // ── Netlink ──
        ProtocolNames {
            canonical: "Netlink",
            xdp2: Some("xdp2_parse_netlink"),
            kernel_struct: Some("nlmsghdr"),
            kernel_header: Some("linux/netlink.h"),
            scapy: Some("NetlinkHeader"),
            tshark: Some("netlink"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 16,
            variable_length: true,
        },
        // ── Legacy ──
        ProtocolNames {
            canonical: "IPX",
            xdp2: Some("xdp2_parse_ipx"),
            kernel_struct: Some("ipxhdr"),
            kernel_header: Some("linux/ipx.h"),
            scapy: Some("IPX"),
            tshark: Some("ipx"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 30,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "AppleTalk",
            xdp2: Some("xdp2_parse_atalk"),
            kernel_struct: Some("atalk_addr"),
            kernel_header: Some("linux/atalk.h"),
            scapy: Some("DDP"),
            tshark: Some("ddp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 5,
            variable_length: false,
        },
        // ── IPv6 Extensions ──
        ProtocolNames {
            canonical: "IPv6_EH",
            xdp2: Some("xdp2_parse_ipv6_eh"),
            kernel_struct: Some("ipv6_opt_hdr"),
            kernel_header: Some("linux/ipv6.h"),
            scapy: Some("IPv6ExtHdrHopByHop"),
            tshark: Some("ipv6.hop"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 2,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "IPv6_ND",
            xdp2: Some("xdp2_parse_ipv6_nd"),
            kernel_struct: Some("nd_msg"),
            kernel_header: Some("linux/neighbour.h"),
            scapy: Some("ICMPv6ND_NS"),
            tshark: Some("icmpv6"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 24,
            variable_length: true,
        },
        // ── IEEE 802.15.4 ──
        ProtocolNames {
            canonical: "IEEE802154",
            xdp2: Some("xdp2_parse_ieee802154"),
            kernel_struct: Some("ieee802154_hdr_fc"),
            kernel_header: Some("linux/ieee802154.h"),
            scapy: Some("Dot15d4"),
            tshark: Some("wpan"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 3,
            variable_length: true,
        },
        // ── MCTP ──
        ProtocolNames {
            canonical: "MCTP",
            xdp2: Some("xdp2_parse_mctp"),
            kernel_struct: Some("mctp_hdr"),
            kernel_header: Some("linux/mctp.h"),
            scapy: Some("MCTP"),
            tshark: Some("mctp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: false,
        },
        // ── Phonet ──
        ProtocolNames {
            canonical: "Phonet",
            xdp2: Some("xdp2_parse_phonet"),
            kernel_struct: Some("phonethdr"),
            kernel_header: Some("linux/phonet.h"),
            scapy: Some("Phonet"),
            tshark: Some("phonet"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 7,
            variable_length: false,
        },
        // ── Generic Netlink ──
        ProtocolNames {
            canonical: "GenNetlink",
            xdp2: Some("xdp2_parse_genetlink"),
            kernel_struct: Some("genlmsghdr"),
            kernel_header: Some("linux/genetlink.h"),
            scapy: Some("GenlMsgHdr"),
            tshark: Some("genl"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "NLAttr",
            xdp2: Some("xdp2_parse_nlattr"),
            kernel_struct: Some("nlattr"),
            kernel_header: Some("linux/netlink.h"),
            scapy: Some("NLAttr"),
            tshark: None,
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: false,
        },
        // ── TIPC ──
        ProtocolNames {
            canonical: "TIPC",
            xdp2: Some("xdp2_parse_tipc"),
            kernel_struct: Some("tipc_basic_hdr"),
            kernel_header: Some("linux/tipc.h"),
            scapy: Some("TIPC"),
            tshark: Some("tipc"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 16,
            variable_length: false,
        },
        // ── Bluetooth Variants ──
        ProtocolNames {
            canonical: "HCI_CMD",
            xdp2: Some("xdp2_parse_hci_cmd"),
            kernel_struct: Some("hci_command_hdr"),
            kernel_header: Some("net/bluetooth/hci.h"),
            scapy: Some("HCI_Command_Hdr"),
            tshark: Some("bthci_cmd"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 3,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "HCI_ACL",
            xdp2: Some("xdp2_parse_hci_acl"),
            kernel_struct: Some("hci_acl_hdr"),
            kernel_header: Some("net/bluetooth/hci.h"),
            scapy: Some("HCI_ACL_Hdr"),
            tshark: Some("bthci_acl"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "HCI_Event",
            xdp2: Some("xdp2_parse_hci_event"),
            kernel_struct: Some("hci_event_hdr"),
            kernel_header: Some("net/bluetooth/hci.h"),
            scapy: Some("HCI_Event_Hdr"),
            tshark: Some("bthci_evt"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 2,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "HCI_SCO",
            xdp2: Some("xdp2_parse_hci_sco"),
            kernel_struct: Some("hci_sco_hdr"),
            kernel_header: Some("net/bluetooth/hci.h"),
            scapy: Some("HCI_SCO_Hdr"),
            tshark: Some("bthci_sco"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 3,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "HCI_ISO",
            xdp2: Some("xdp2_parse_hci_iso"),
            kernel_struct: Some("hci_iso_hdr"),
            kernel_header: Some("net/bluetooth/hci.h"),
            scapy: Some("HCI_ISO_Hdr"),
            tshark: Some("bthci_iso"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: false,
        },
        // ── EAPOL ──
        ProtocolNames {
            canonical: "EAPOL",
            xdp2: Some("xdp2_parse_eapol"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("EAPOL"),
            tshark: Some("eapol"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: false,
        },
        // ── CAN XL ──
        ProtocolNames {
            canonical: "CAN_XL",
            xdp2: Some("xdp2_parse_canxl"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("CANXL"),
            tshark: Some("can"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 12,
            variable_length: false,
        },
        // ── Management & Industrial ──
        ProtocolNames {
            canonical: "TRILL",
            xdp2: Some("xdp2_parse_trill"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("TRILL"),
            tshark: Some("trill"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 6,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "BATMAN",
            xdp2: Some("xdp2_parse_batman"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("BATMAN_OGM"),
            tshark: Some("batadv"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 10,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "DSA",
            xdp2: Some("xdp2_parse_dsa"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("DSA"),
            tshark: Some("dsa"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "EDSA",
            xdp2: Some("xdp2_parse_edsa"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("EDSA"),
            tshark: Some("edsa"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 10,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "CFM",
            xdp2: Some("xdp2_parse_cfm"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("CFM"),
            tshark: Some("cfm"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "FIP",
            xdp2: Some("xdp2_parse_fip"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("FIP"),
            tshark: Some("fip"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 10,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "MAC_Control",
            xdp2: Some("xdp2_parse_mac_control"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("MACControlPause"),
            tshark: Some("macc"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "MVRP",
            xdp2: Some("xdp2_parse_mvrp"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("MVRP"),
            tshark: Some("mvrp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 1,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "NC_SI",
            xdp2: Some("xdp2_parse_ncsi"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("NCSI"),
            tshark: Some("ncsi"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "Slow_Protocols",
            xdp2: Some("xdp2_parse_slow"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("SlowProtocol"),
            tshark: Some("slow"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 2,
            variable_length: false,
        },
        // ── Remaining Legacy/Niche ──
        ProtocolNames {
            canonical: "PROFINET",
            xdp2: Some("xdp2_parse_profinet"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("ProfinetIO"),
            tshark: Some("pn_rt"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 2,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "X25",
            xdp2: Some("xdp2_parse_x25"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("X25"),
            tshark: Some("x25"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 3,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "ATM",
            xdp2: Some("xdp2_parse_atm"),
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("ATM"),
            tshark: Some("atm"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 5,
            variable_length: false,
        },
        // ── Redundancy ──
        ProtocolNames {
            canonical: "VRRP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("VRRPv3"),
            tshark: Some("vrrp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "HSRP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("HSRP"),
            tshark: Some("hsrp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 20,
            variable_length: false,
        },
        // ── Discovery ──
        ProtocolNames {
            canonical: "CDP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("CDPv2_HDR"),
            tshark: Some("cdp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: true,
        },
        // ── Routing ──
        ProtocolNames {
            canonical: "RIP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("RIP"),
            tshark: Some("rip"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "OSPF",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("OSPF_Hdr"),
            tshark: Some("ospf"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 24,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "ISIS",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("ISIS_CommonHdr"),
            tshark: Some("isis"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "BGP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("BGPHeader"),
            tshark: Some("bgp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 19,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "EIGRP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("EIGRP"),
            tshark: Some("eigrp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 20,
            variable_length: true,
        },
        // ── Bluetooth (upper layers) ──
        ProtocolNames {
            canonical: "BT_ATT",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("ATT_Hdr"),
            tshark: Some("btatt"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 1,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "BT_SMP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("SM_Hdr"),
            tshark: Some("btsmp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 1,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "BT_RFCOMM",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("RFCOMM_Hdr"),
            tshark: Some("btrfcomm"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "BT_BNEP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("BNEP"),
            tshark: Some("btbnep"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 3,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "BT_SDP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("SDP_Hdr"),
            tshark: Some("btsdp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 3,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "BT_AVDTP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("AVDTP_Hdr"),
            tshark: Some("btavdtp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 2,
            variable_length: true,
        },
        // ── InfiniBand (extended transport headers) ──
        ProtocolNames {
            canonical: "IB_DETH",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("IB_DETH"),
            tshark: Some("infiniband.deth"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "IB_RETH",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("IB_RETH"),
            tshark: Some("infiniband.reth"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 16,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "IB_AETH",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("IB_AETH"),
            tshark: Some("infiniband.aeth"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "IB_RDETH",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("IB_RDETH"),
            tshark: Some("infiniband.rdeth"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "IB_AtomicETH",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("IB_AtomicETH"),
            tshark: Some("infiniband.atomiceth"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 28,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "IB_ImmDt",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("IB_ImmDt"),
            tshark: Some("infiniband.immdt"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "IB_MAD",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("IB_MAD"),
            tshark: Some("infiniband.mad"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 24,
            variable_length: true,
        },
        // ── Multicast ──
        ProtocolNames {
            canonical: "IGMPv3_Query",
            xdp2: None,
            kernel_struct: Some("igmpv3_query"),
            kernel_header: Some("linux/igmp.h"),
            scapy: Some("IGMPv3"),
            tshark: Some("igmp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 12,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "IGMPv3_Report",
            xdp2: None,
            kernel_struct: Some("igmpv3_report"),
            kernel_header: Some("linux/igmp.h"),
            scapy: Some("IGMPv3mr"),
            tshark: Some("igmp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "MLD",
            xdp2: None,
            kernel_struct: Some("mld_msg"),
            kernel_header: Some("net/mld.h"),
            scapy: Some("ICMPv6MLQuery"),
            tshark: Some("icmpv6"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 24,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "MLDv2_Query",
            xdp2: None,
            kernel_struct: Some("mld2_query"),
            kernel_header: Some("net/mld.h"),
            scapy: Some("ICMPv6MLQuery2"),
            tshark: Some("icmpv6"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 28,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "MLDv2_Report",
            xdp2: None,
            kernel_struct: Some("mld2_report"),
            kernel_header: Some("net/mld.h"),
            scapy: Some("ICMPv6MLReport2"),
            tshark: Some("icmpv6"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: true,
        },
        // ── Media/Streaming ──
        ProtocolNames {
            canonical: "MPEG_TS",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("MPEG_TS"),
            tshark: Some("mp2t"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 188,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "RTP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("RTP"),
            tshark: Some("rtp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 12,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "SRT",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("SRT"),
            tshark: Some("srt"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 16,
            variable_length: true,
        },
        // ── UDP Application Protocols ──
        ProtocolNames {
            canonical: "DNS",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("DNS"),
            tshark: Some("dns"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 12,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "NTP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("NTPHeader"),
            tshark: Some("ntp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 48,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "SNMP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("SNMP"),
            tshark: Some("snmp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 10,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "DHCP",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("BOOTP"),
            tshark: Some("dhcp"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 236,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "DHCPv6",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("DHCP6"),
            tshark: Some("dhcpv6"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "QUIC",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("QUIC_Initial"),
            tshark: Some("quic"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 20,
            variable_length: true,
        },
        // ── Storage Protocols ──
        ProtocolNames {
            canonical: "iSCSI",
            xdp2: None,
            kernel_struct: Some("iscsi_hdr"),
            kernel_header: Some("scsi/iscsi_proto.h"),
            scapy: Some("iSCSI_BHS"),
            tshark: Some("iscsi"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 48,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "NVMe",
            xdp2: None,
            kernel_struct: Some("nvme_common_command"),
            kernel_header: Some("linux/nvme.h"),
            scapy: Some("NVMe_Command"),
            tshark: Some("nvme"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 64,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "FC",
            xdp2: None,
            kernel_struct: Some("fc_frame_header"),
            kernel_header: Some("uapi/scsi/fc/fc_fs.h"),
            scapy: Some("FCFrame"),
            tshark: Some("fc"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 24,
            variable_length: false,
        },
        ProtocolNames {
            canonical: "SCSI",
            xdp2: None,
            kernel_struct: Some("scsi_lun"),
            kernel_header: Some("scsi/scsi_proto.h"),
            scapy: Some("SCSI_LUN"),
            tshark: Some("scsi"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 8,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "iSER",
            xdp2: None,
            kernel_struct: Some("iser_ctrl"),
            kernel_header: Some("scsi/iser.h"),
            scapy: Some("iSER_Ctrl"),
            tshark: Some("iser"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 28,
            variable_length: false,
        },
        // ── Network Storage Protocols ──
        ProtocolNames {
            canonical: "NFS",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("NFS_Call"),
            tshark: Some("nfs"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "SMB",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("SMB_Header"),
            tshark: Some("smb"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 32,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "SMB2",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("SMB2_Header"),
            tshark: Some("smb2"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 64,
            variable_length: true,
        },
        ProtocolNames {
            canonical: "ONC_RPC",
            xdp2: None,
            kernel_struct: None,
            kernel_header: None,
            scapy: Some("RPC"),
            tshark: Some("rpc"),
            etherparse_struct: None,
            etherparse_file: None,
            libpcap_name: None,
            libpcap_file: None,
            min_header_bytes: 4,
            variable_length: true,
        },
    ]
}

/// Look up a protocol by canonical name (case-insensitive).
pub fn find_by_canonical(name: &str) -> Option<ProtocolNames> {
    let lower = name.to_lowercase();
    protocol_table()
        .into_iter()
        .find(|p| p.canonical.to_lowercase() == lower)
}

/// Look up a protocol by its XDP2 parse node name.
pub fn find_by_xdp2_name(name: &str) -> Option<ProtocolNames> {
    protocol_table()
        .into_iter()
        .find(|p| p.xdp2 == Some(name))
}

/// Look up a protocol by its kernel struct name.
pub fn find_by_kernel_struct(name: &str) -> Option<ProtocolNames> {
    protocol_table()
        .into_iter()
        .find(|p| p.kernel_struct == Some(name))
}

/// Look up a protocol by its Scapy class name.
pub fn find_by_scapy_name(name: &str) -> Option<ProtocolNames> {
    protocol_table()
        .into_iter()
        .find(|p| p.scapy == Some(name))
}

/// Look up a protocol by its tshark filter name.
pub fn find_by_tshark_name(name: &str) -> Option<ProtocolNames> {
    protocol_table()
        .into_iter()
        .find(|p| p.tshark == Some(name))
}

/// Look up a protocol by its etherparse struct name.
pub fn find_by_etherparse_struct(name: &str) -> Option<ProtocolNames> {
    protocol_table()
        .into_iter()
        .find(|p| p.etherparse_struct == Some(name))
}

/// Look up a protocol by its libpcap name.
pub fn find_by_libpcap_name(name: &str) -> Option<ProtocolNames> {
    protocol_table()
        .into_iter()
        .find(|p| p.libpcap_name == Some(name))
}

/// Build a HashMap from source-specific name → canonical name.
///
/// `source` must be one of: "xdp2", "kernel", "scapy", "tshark", "etherparse"
pub fn source_to_canonical_map(source: &str) -> HashMap<String, String> {
    let table = protocol_table();
    let mut map = HashMap::new();
    for p in &table {
        let name = match source {
            "xdp2" => p.xdp2.map(|s| s.to_string()),
            "kernel" => p.kernel_struct.map(|s| s.to_string()),
            "scapy" => p.scapy.map(|s| s.to_string()),
            "tshark" => p.tshark.map(|s| s.to_string()),
            "etherparse" => p.etherparse_struct.map(|s| s.to_string()),
            "libpcap" => p.libpcap_name.map(|s| s.to_string()),
            _ => None,
        };
        if let Some(n) = name {
            map.insert(n, p.canonical.to_string());
        }
    }
    map
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_find_ipv4_by_canonical() {
        let p = find_by_canonical("IPv4").unwrap();
        assert_eq!(p.kernel_struct, Some("iphdr"));
        assert_eq!(p.scapy, Some("IP"));
        assert_eq!(p.tshark, Some("ip"));
        assert_eq!(p.min_header_bytes, 20);
        assert!(p.variable_length);
    }

    #[test]
    fn test_find_by_canonical_case_insensitive() {
        assert!(find_by_canonical("ipv4").is_some());
        assert!(find_by_canonical("IPV4").is_some());
        assert!(find_by_canonical("tcp").is_some());
    }

    #[test]
    fn test_find_by_xdp2_name() {
        let p = find_by_xdp2_name("xdp2_parse_tcp_notlvs").unwrap();
        assert_eq!(p.canonical, "TCP");
    }

    #[test]
    fn test_find_by_kernel_struct() {
        let p = find_by_kernel_struct("tcphdr").unwrap();
        assert_eq!(p.canonical, "TCP");
    }

    #[test]
    fn test_find_by_scapy_name() {
        let p = find_by_scapy_name("Ether").unwrap();
        assert_eq!(p.canonical, "Ethernet");
    }

    #[test]
    fn test_source_to_canonical_map() {
        let map = source_to_canonical_map("scapy");
        assert_eq!(map.get("IP"), Some(&"IPv4".to_string()));
        assert_eq!(map.get("TCP"), Some(&"TCP".to_string()));
        assert_eq!(map.get("Ether"), Some(&"Ethernet".to_string()));
    }

    #[test]
    fn test_protocol_table_no_duplicates() {
        let table = protocol_table();
        let mut seen = std::collections::HashSet::new();
        for p in &table {
            assert!(
                seen.insert(p.canonical),
                "Duplicate canonical name: {}",
                p.canonical
            );
        }
    }
}
