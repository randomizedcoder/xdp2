...
- [ ] **D7.** Userspace loader `xdp2-flow-loader` (Rust).
  ...
  - [ ] **D7c** — `Loader::attach` — attach entry program to the flow_dissector hook in a target netns.
  - [ ] **D7d** — slow-path `.o` handling: load + install entry program into `CHAIN_DYNAMIC` (unblocks D6).
