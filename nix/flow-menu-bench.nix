...
let
  ...
  # Synthetic per-shape pcaps with known-by-construction inner flows, plus
  # the matching golden inner-5-tuple CSVs (gold/) — the ground-truth for the
  # parity_test -D Gold gate. Generated by
  # kernel-patches/series6-common-case/perf/gold/GEN.py; the proto_audit
  # header templates lacked valid inner flows (no third-party dissector,
  # tshark included, would descend them), so they can't gate or benchmark
  # the descents.
  corpus = pkgs.runCommand "xdp2-flow-menu-corpus" { } ''
    mkdir -p $out/gold
    cp ${../data/pcaps/flow-menu/eth_ip.pcap} $out/eth_ip.pcap
    cp ${../data/pcaps/flow-menu/vlan.pcap} $out/vlan.pcap
    cp ${../data/pcaps/flow-menu/qinq.pcap} $out/qinq.pcap
    cp ${../data/pcaps/flow-menu/mpls.pcap} $out/mpls.pcap
    cp ${../data/pcaps/flow-menu/ipip.pcap} $out/ipip.pcap
    cp ${../data/pcaps/flow-menu/gre.pcap} $out/gre.pcap
    cp ${../data/pcaps/flow-menu/pppoe.pcap} $out/pppoe.pcap
    cp ${../data/pcaps/flow-menu/vxlan.pcap} $out/vxlan.pcap
    cp ${../data/pcaps/flow-menu/geneve.pcap} $out/geneve.pcap
    cp ${../data/pcaps/flow-menu/gtpu.pcap} $out/gtpu.pcap
    cp ${../data/pcaps/flow-menu/gue.pcap} $out/gue.pcap
    cp ${../data/pcaps/flow-menu/fou.pcap} $out/fou.pcap
    cp -r ${../kernel-patches/series6-common-case/perf/gold}/. $out/gold/
  '';
