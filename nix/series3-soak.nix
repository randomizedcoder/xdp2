...
#
# Series 3 — long-running real-traffic soak across the Pi pair fleet.
#
# Static 10-cell matrix (pair × proto × sysctl), each cell runs a
# single elephant for the configured duration (default 3600 s).
# 1 Hz vcgencmd sidecar (temp / freq / throttled bits) on BOTH
# endpoints, per-second iperf interval JSON / CSV. Per-cell
# summary.json captures throughput, drift, retransmits, CPU% and
# thermal cleanliness; final aggregate.csv across the 10 cells.
#
# Repeatable companion to nix/series3-traffic-ab.nix (which is
# short-burst interleaved A/B; this is single-long-elephant per
# cell, ~10 h total wall clock).
#
# The matrix:
#
#   cell  sender  recv    proto              sysctl
...
#
# Usage:
#   # Default: 10 cells × 3600 s = ~10.3 h wall clock
#   nix run .#series3-soak
#
#   # Shorter cells (e.g. validation pass):
#   DUR=300 COOLDOWN=30 nix run .#series3-soak
#
#   # Custom matrix host-mapping:
#   PI5_1=pi5-1 PI5_2=pi5-2 PI4_1=pi4-1 PI3_1=pi3-1 \
#     nix run .#series3-soak
#
# Pre-conditions:
#   - root SSH from the calling host to all four Pis.
#   - All Pis have the series-3 patched kernel booted, sysctl
#     net.core.flow_dissector_fastpath registered.
#   - Receivers have iperf3 + iperf2 binaries materialised; the
#     harness checks and auto-runs nix build if missing.
#   - Receivers have iperf3 / iperf2 firewall ports open in
#     nixos-fw; the harness inserts the rules idempotently.
#   - For tcpreplay cells (9, 10): pi5-2 has the dst-MAC-rewritten
#     PCAP at /root/replay/pcaps/vxlan-k8s-pure-pi3-1.pcap.
#     This is the same PCAP Phase H rewrote; if missing, regenerate
#     with `nix run .#series3-tcpreplay-prep` (TBD) or by hand:
#       tcprewrite --enet-dmac=<pi3-1-MAC> --enet-smac=<pi5-2-MAC> \
#         --infile=/root/replay/pcaps/vxlan-k8s-pure.pcap \
#         --outfile=/root/replay/pcaps/vxlan-k8s-pure-pi3-1.pcap
#
# Outputs (in OUT, default perf-results/<YYYY-MM-DD>-series3-soak/):
#   - run.log                                        master timeline
#   - cells/cell-NN-{sender}-{recv}-{proto}-sysctl{0|1}/
#       * iperf.json / iperf.csv / replay.log / mpstat.json
#       * sidecar-{sender}.csv  sidecar-{recv}.csv
#       * summary.json
#   - aggregate.csv                                  10-row table
#
# Thermal policy:
#   - 1 Hz sidecar on both endpoints; post-hoc tagging
#     thermal_clean = (low 16 bits of throttled were 0 throughout).
#   - Pre-cell check: if receiver temp > TEMP_GATE (default 65 °C),
#     sleep an extra 300 s before starting the cell.
#   - No mid-cell auto-abort. Throttled cells are documented in the
